...
def insert_into_discovery_if_not_exists(url, db_name='db_websites.db'):
    """
    Inserts the url into tb_discovery if it doesn't already exist.
    ...
    if count == 0:
        ...
        cursor.execute('INSERT INTO tb_discovery (url) VALUES (?)', (url,))
        conn.commit()
        print(f"Inserted '{url}' into 'tb_discovery'.")
    else:
